...
namespace Negocio
{
    ...
    {
        public List<Marca> listar()  //Trae un listado/lista de  objetos (marcas)
        {
            List<Marca> lista = new List<Marca>(); //Crea una lista vacía para guardar las marcas.
            ...
            try
            {
                datos.setearConsulta("select Id, descripcion from marcas"); //Le dice a AccesoDatos qué consulta SQL tiene que ejecutar
                ...
                while (datos.Lector.Read()) //Cada vez que encuentra un registro en la tabla: Crea un objeto Marca nuevo (aux).
